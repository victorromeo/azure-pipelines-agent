...
namespace Agent.Cli
{
    ...
    public class LocalPipelineParser
    {
    
        public LocalPipelineParser(ITraceWriter trace, IFileProvider fileProvider, ParseOptions options)
        {
            if (trace == null)
            {
        ...
        public Process Deserialize(String defaultRoot, String path,
            IDictionary<String, Object> mustacheContext, CancellationToken cancellationToken)
        {
            Int32 fileCount = 0;

            // Load the target file.
            path = m_fileProvider.ResolvePath(defaultRoot: defaultRoot, path: path);
            PipelineFile<Process> processFile =
                LoadFile<Process, ProcessConverter>(path, mustacheContext, cancellationToken, ref fileCount);
            Process process = processFile.Object;
            ResolveTemplates(process, defaultRoot: processFile.Directory, cancellationToken: cancellationToken,
        ...
        /// </summary>
        [EditorBrowsable(EditorBrowsableState.Never)]
        public String DeserializeAndSerialize(String defaultRoot, String path,
            IDictionary<String, Object> mustacheContext, CancellationToken cancellationToken)
        {
            Int32 fileCount = 0;
        ...
        internal Process LoadInternal(String defaultRoot, String path, IDictionary<String, Object> mustacheContext,
                ...
                foreach (Phase phase in process.Phases)
                {
                    if (phase.Steps != null && phase.Steps.Count > 0)
                    {
                        if (phase.Steps[0] is CheckoutStep)
                        {
                            ...
                            foreach (Variable variable in (phase.Steps[0] as CheckoutStep).GetVariables(
                                process.Resources))
                            {
                        ...
                        if (phase.Steps.Any(x => x is CheckoutStep))
                        {
                            throw new InvalidOperationException(
                                $"Step '{YamlConstants.Checkout}' is currently only supported as the first step within a phase.");
                        }
                    }
                }
            }
        ...
        private PipelineFile<TObject> LoadFile<TObject, TConverter>(String path,
            IDictionary<String, Object> mustacheContext, CancellationToken cancellationToken, ref Int32 fileCount)
            where TConverter : IYamlTypeConverter, new()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        frontMatter =
                            frontMatterDeserializer.Deserialize<IDictionary<String, Object>>(
                                frontMatterBuilder.ToString());
                    }
                    catch (Exception ex)
                    {
                ...
                cancellationToken.ThrowIfCancellationRequested();
                var mustacheParser =
                    new MustacheTemplateParser(useDefaultHandlebarHelpers: true, useCommonTemplateHelpers: true);
                ...
                catch (System.OperationCanceledException ex) when (mustacheCancellationTokenSource
                    .IsCancellationRequested && !cancellationToken.IsCancellationRequested)
                {
                    throw new System.OperationCanceledException(
                        TaskResources.MustacheEvaluationTimeout(path, m_options.MustacheEvaluationTimeout.TotalSeconds),
                        ex);
                }

                m_trace.Verbose("{0}",
                    new TraceFileContent($"{file.Name} after mustache replacement", mustacheReplaced));
            }
            finally
            {
                reader?.Dispose();
        ...
        private void ResolveTemplates(Process process, String defaultRoot, CancellationToken cancellationToken,
            ref Int32 fileCount)
        {
            if (process.Template != null)
            {
                ...
                String templateFilePath =
                    m_fileProvider.ResolvePath(defaultRoot: defaultRoot, path: process.Template.Name);
                PipelineFile<ProcessTemplate> templateFile =
                    LoadFile<ProcessTemplate, ProcessTemplateConverter>(templateFilePath, process.Template.Parameters,
                        cancellationToken, ref fileCount);
                ProcessTemplate template = templateFile.Object;

                // Resolve template references within the template.
                if (template.Phases != null)
                {
                    ResolveTemplates(template.Phases, defaultRoot: templateFile.Directory,
                        cancellationToken: cancellationToken, fileCount: ref fileCount);
                }
                else if (template.Steps != null)
                {
                    ResolveTemplates(template.Steps, defaultRoot: templateFile.Directory,
                        cancellationToken: cancellationToken, fileCount: ref fileCount);
                }
        ...
        private void ResolveTemplates(IList<IPhase> phases, String defaultRoot, CancellationToken cancellationToken,
            ref Int32 fileCount)
        {
            for (int i = 0; i < (phases?.Count ?? 0);)
            {
                if (phases[i] is PhasesTemplateReference)
                {
                    ...
                    String templateFilePath =
                        m_fileProvider.ResolvePath(defaultRoot: defaultRoot, path: reference.Name);
                    PipelineFile<PhasesTemplate> templateFile =
                        LoadFile<PhasesTemplate, PhasesTemplateConverter>(templateFilePath, reference.Parameters,
                            cancellationToken, ref fileCount);
        ...
        private void ResolveTemplates(IList<IVariable> variables, String defaultRoot,
            CancellationToken cancellationToken, ref Int32 fileCount)
        {
            for (int i = 0; i < (variables?.Count ?? 0);)
            {
                if (variables[i] is VariablesTemplateReference)
                {
                    ...
                    String templateFilePath =
                        m_fileProvider.ResolvePath(defaultRoot: defaultRoot, path: reference.Name);
                    PipelineFile<VariablesTemplate> templateFile =
                        LoadFile<VariablesTemplate, VariablesTemplateConverter>(templateFilePath, reference.Parameters,
                            cancellationToken, ref fileCount);
        ...
        private void ResolveTemplates(IList<IStep> steps, String defaultRoot, CancellationToken cancellationToken,
            ref Int32 fileCount)
        {
            for (int i = 0; i < (steps?.Count ?? 0);)
            {
                if (steps[i] is StepsTemplateReference)
                {
                    ...
                    String templateFilePath =
                        m_fileProvider.ResolvePath(defaultRoot: defaultRoot, path: reference.Name);
                    PipelineFile<StepsTemplate> templateFile =
                        LoadFile<StepsTemplate, StepsTemplateConverter>(templateFilePath, reference.Parameters,
                            cancellationToken, ref fileCount);
        ...
        private static void ApplyStepOverrides(PhasesTemplateReference reference, PhasesTemplate template)
        {
            ...
            var byPhaseNames =
                ...
                .Join(inner: (template.Phases ?? new List<IPhase>(0)).Cast<Phase>(),
                    ...
                    resultSelector: (PhaseSelector phaseSelector, Phase phase) =>
                        new {Selector = phaseSelector, Phase = phase})
                .ToArray();
        ...
        private static void ApplyStepOverrides(IDictionary<String, IList<ISimpleStep>> stepOverrides,
            IList<IStep> steps)
        {
        ...
        private static List<ProcessResource> MergeResources(IList<ProcessResource> overrides,
            IList<ProcessResource> imports)
        {
