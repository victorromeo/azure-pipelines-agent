...
function cmd_build ()
{
    ...
    if  [[ "$ADO_ENABLE_LOGISSUE" == "true" ]]; then
        ...
        dotnet msbuild -p:NuGetInteractive="true" -t:${TARGET} -p:PackageRuntime="${RUNTIME_ID}" -p:PackageType="${PACKAGE_TYPE}" -p:BUILDCONFIG="${BUILD_CONFIG}" -p:AgentVersion="${AGENT_VERSION}" -p:LayoutRoot="${LAYOUT_DIR}" -p:CodeAnalysis="true" \
