...
namespace Microsoft.VisualStudio.Services.Agent.Listener.Configuration
{
    ...
    public sealed class ConfigurationManager : AgentService, IConfigurationManager
    {
        ...
        public async Task ConfigureAsync(CommandSettings command)
        {
            ...
            string agentType;
            if (command.args.Configure != null && (command.args.Configure.DeploymentGroup || command.args.Configure.MachineGroup))
            {
                agentType = Constants.Agent.AgentConfigurationProvider.DeploymentAgentConfiguration;
            }
            else if (command.args.Configure != null && command.args.Configure.DeploymentPool)
            {
                agentType = Constants.Agent.AgentConfigurationProvider.SharedDeploymentAgentConfiguration;
            }
            else if (command.args.Configure != null && command.args.Configure.EnvironmentVMResource)
            {
            ...
            while (true)
            {
                ...
                catch (Exception e) when (!command.args.Configure.Unattended)
                {
                    _term.WriteError(e);
            ...
            while (true)
            {
                ...
                if (agent != null)
                {
                    ...
                    else if (command.args.Configure.Unattended)
                    {
            ...
            if (PlatformUtil.RunningOnWindows && command.args.Configure != null && command.args.Configure.GitUseSChannel)
            {
        ...
        private ICredentialProvider GetCredentialProvider(CommandSettings command, string serverUrl)
        {
            ...
            if (provider.RequireInteractive && command.args.Configure.Unattended)
            {
