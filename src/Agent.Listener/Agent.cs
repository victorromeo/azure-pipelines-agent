    ...
    public sealed class Agent : AgentService, IAgent
    {
        ...
        public async Task<int> ExecuteCommand(CommandSettings command)
        {
            try
            {
                ...
                if (command.args.Run != null & command.args.Run.Help)
                {
                    PrintUsage(command);
                    return Constants.Agent.ReturnCode.Success;
                }

                if (command.args.Run != null & command.args.Run.Version)
                {
                    _term.WriteLine(BuildConstants.AgentPackage.Version);
                    return Constants.Agent.ReturnCode.Success;
                }

                if (command.args.Run != null & command.args.Run.Commit)
                {
                    _term.WriteLine(BuildConstants.Source.CommitHash);
                    return Constants.Agent.ReturnCode.Success;
                }

                if (command.args.Run != null & command.args.Run.Diagnostics)
                {
                    PrintBanner();
                ...
                if (command.args.Configure != null)
                {
                    PrintBanner();
                    try
                    {
                ...
                return await RunAsync(settings, command.args.Run != null ? command.args.Run.RunOnce : false);
            }
            finally
            {
                _term.CancelKeyPress -= CtrlCHandler;
        ...
        private void PrintUsage(CommandSettings command)
        {
            ...
            else if (command.args.Remove != null)
            {
