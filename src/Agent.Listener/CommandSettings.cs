...
namespace Microsoft.VisualStudio.Services.Agent.Listener
{
    public sealed class CommandSettings
    {
        ...
        private BaseCommand Base { get; set; }
        ...
        private void ParseArguments(string[] args, bool ignoreErrors)
        {
            ...
            }))
            {
                ...
                // Parse ignoring any verbs (Run is the default verb)
                parser
                    .ParseArguments<BaseCommand>(args)
                    .WithParsed(
                       x =>
                       {
                           Base = x;
                ...
                // Parse verbs
                // commit && version require no further parsing
                if (Base == null || (!Base.Commit && !Base.Version))
                {
                    parser
                        ...
                        .WithNotParsed(
                            errors =>
                            {
                                //Base.Help || Base.Once (THen bad verb)
                                // Otherwise just no verb

                                ParseErrors = errors.Where(x => x.Tag != ErrorType.BadVerbSelectedError ||
                                    x.Tag != ErrorType.NoVerbSelectedError);
                            });
                }
            }
        }
